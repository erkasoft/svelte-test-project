...
{#if photo}
      ...
      <div
class="photo-card relative overflow-hidden  cursor-pointer  hover:opacity-50 hover:cursor-pointer" on:click={modalHandler}
      >
<div  style:width="250px" style:height="250px" style:background-image={`url(${photo})`} class="bg-cover bg-center w-full h-full" />
...
{#if isPhotoModalOpen }
    ...
    <div
      ...
      role='button'
    >
      <div class='absolute w-screen h-screen bg-primary-800 opacity-60'></div>
      ...
      >
        <div class='flex items-center justify-center w-full p-2 bg-transparent' style={{ height: '70%'}}>
          <div class='absolute top-0 right-0 z-50 flex items-center mt-4 mr-4 text-sm text-white cursor-pointer modal-close'>
            <button
